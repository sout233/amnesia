<script lang="ts">
	...
	let css = $derived.by(() => {
		if (level === 'error') return 'bg-error text-error-content border border-error/20';
		if (level === 'warning') return 'bg-warning text-warning-content border border-warning/20';
		if (level === 'success') return 'bg-success text-success-content border border-success/20';
